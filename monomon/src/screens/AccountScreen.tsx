...
export default function AccountScreen() {
  ...
  const carregarUsuarioEEprestimos = async () => {
    ...
    try {
      ...
      const meus = todos.filter(
        (e: any) => e.usuarioId === GlobalStore.user.id,
      );
      setMeusEmprestimos(meus);

      if (GlobalStore.user.tipo === "ADMIN") {
        const pend = todos.filter(
          (e: any) => String(e.status).toUpperCase() === "PENDENTE",
        );
        setPendentes(pend);

        const ativosDeOutros = todos.filter(
          (e: any) => String(e.status).toUpperCase() !== "DEVOLVIDO",
        );
  ...
  const getStatusBadge = (emp: any) => {
    ...
    if (statusText === "EMPRESTADO") {
      ...
      if (
        emp.dataMaximaDevolucao &&
        new Date(emp.dataMaximaDevolucao) < new Date()
      ) {
        statusText = "ATRASADO";
        bgColor = "#ef4444"; // Red
      }
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {meusEmprestimos.map((emp) => {
            const isAtivo =
              String(emp.status).toUpperCase() === "EMPRESTADO" ||
              (String(emp.status).toUpperCase() === "EMPRESTADO" &&
                emp.dataMaximaDevolucao &&
                new Date(emp.dataMaximaDevolucao) < new Date());
              ...
              <View key={emp.id} style={styles.loanItem}>
                <View style={{ flex: 1 }}>
                  ...
                  {emp.dataMaximaDevolucao && (
                    <Text style={styles.loanDate}>
                      Prazo:{" "}
                      {new Date(emp.dataMaximaDevolucao).toLocaleDateString()}
                    </Text>
                  )}
                  {getStatusBadge(emp)}
                </View>
        ...
        {userRole === "ADMIN" && (
          <View>
            <View style={[styles.card, { marginTop: 16 }]}>
              <View style={styles.cardHeader}>
                ...
                <Text style={styles.cardTitle}>
                  EMPRÉSTIMOS PENDENTES (APROVAÇÃO)
                </Text>
              </View>
              <Text style={styles.cardDesc}>
                {pendentes.length === 0
              ...
              {pendentes.map((emp) => (
                <View key={emp.id} style={styles.loanItem}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.loanDate}>
                      Leitor: {getUsuarioNome(emp.usuarioId)}
                    </Text>
                    <Text style={styles.loanDate}>
                      Dias solicitados: {emp.diasDesejados || 15} dias
                    </Text>
                  </View>
                  <TouchableOpacity
                    style={[
            ...
            <View style={[styles.card, { marginTop: 16 }]}>
              ...
              <Text style={styles.cardDesc}>
                Como administrador atual, você tem permissão para cadastrar
                novos funcionários.
              </Text>
              ...
              >
                <Text style={styles.primaryButtonText}>
                  {loading ? "CRIANDO..." : "CONCEDER ACESSO ADMIN"}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
