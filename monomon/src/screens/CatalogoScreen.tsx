...
export default function CatalogoScreen() {
  ...
  const autoresFiltrados =
    autorBusca && !autorSelecionado
      ? autores.filter((a) =>
          a.nome.toLowerCase().includes(autorBusca.toLowerCase()),
        )
      : [];
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {livros.map((livro) => {
          const autorNome =
            autores.find((a) => a.id === livro.autorId)?.nome ||
            `Autor ID: ${livro.autorId}`;
          return (
            <TouchableOpacity
              key={livro.id}
              style={styles.bookListItem}
              onPress={() => handleOpenDetails(livro)}
            >
              ...
              <View style={styles.bookListInfo}>
                ...
                <Text style={styles.bookListSubtitle}>
                  {livro.generos} • {autorNome}
                </Text>
                <View style={styles.bookListMeta}>
                  <Text style={styles.bookListQty}>
                    Qtd: {livro.qntd} | Dias Máx: {livro.diasMaximos || 15}
                  </Text>
                </View>
              </View>
              {userRole === "ADMIN" ? (
                <TouchableOpacity
                  onPress={() => handleOpenAdmin(livro)}
                  style={{ padding: 8 }}
                >
      ...
      {detailsVisible && selectedBook && (
        <View style={styles.drawer}>
          <View style={styles.drawerHeader}>
            <View style={{ flex: 1 }}>
              <View style={styles.drawerTag}>
                <Text style={styles.drawerTagText}>
                  {selectedBook.generos || "GÊNERO"}
                </Text>
              </View>
              <Text style={styles.drawerTitle}>{selectedBook.nome}</Text>
              <Text style={styles.drawerAuthor}>
                {autores.find((a) => a.id === selectedBook.autorId)?.nome ||
                  `Autor ID: ${selectedBook.autorId}`}
              </Text>
            </View>
            <TouchableOpacity
          ...
          <View style={styles.drawerStats}>
            <Text style={styles.drawerStatsText}>
              Disponíveis:{" "}
              <Text style={{ color: COLORS.primary }}>{selectedBook.qntd}</Text>
            </Text>
            <Text style={styles.drawerStatsText}>
              Max Dias:{" "}
              <Text style={{ color: COLORS.primary }}>
                {selectedBook.diasMaximos || 15}
              </Text>
            </Text>
          </View>
            ...
            {selectedBook.qntd <= 0 ? (
              ...
              >
                <Text style={styles.reserveButtonText}>
                  LIVRO INDISPONÍVEL (SEM ESTOQUE)
                </Text>
              </TouchableOpacity>
            ) : parseInt(diasDesejados, 10) >
              (selectedBook.diasMaximos || 15) ? (
              <View style={{ flex: 1 }}>
                <Text
                  style={{
                ...
                >
                  Você não pode pedir o livro por {diasDesejados} dias (Máximo
                  de {selectedBook.diasMaximos || 15}).
                </Text>
                <TouchableOpacity
                  style={[
                    styles.reserveButton,
            ...
            <View style={styles.modalContent}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  placeholder="Ex: Aventura, Sci-Fi"
                  placeholderTextColor={COLORS.textSecondary}
                />
              </View>
